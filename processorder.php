...
<body>
    ...
    // Debugging output
    echo "Trying to open: " . $document_root . "/../orders/orders.txt<br />";

    // Open file for appending
    try {
        if (!($fp = @fopen("$document_root/../orders/orders.txt", 'ab'))) {
            throw new fileOpenException("Could not open file for writing.");
        }

        if (!flock($fp, LOCK_EX)) {
            throw new fileLockException("Could not lock file.");
        }
